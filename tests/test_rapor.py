...
def test_mesaj_sayilari_dogru_gosterir():
    metin = _mesaj({"giden_mesaj": 12, "giris_token": 4000, "cikis_token": 1500})
    assert "Gönderilen mesaj (WhatsApp): 12" in metin
...
def test_mesaj_fiyat_icermez():
    ...
    metin = _mesaj({"giden_mesaj": 0, "giris_token": 0, "cikis_token": 0})
    assert "$" not in metin
